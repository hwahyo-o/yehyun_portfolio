...
## 2026-08-13 Gallery Swiper 무한 루프 · 자동재생 개선

### 범위와 비밀정보 규칙

- 대상: 홈 화면의 `index.html > .gallery-hero` 내부 배너만.
- 비대상: Cloudflare Worker, D1 스키마, Firebase, Google Drive, 콘텐츠 저장 및 인증 흐름.
- 이 절과 관련 커밋·PR·검증 기록에는 API 키, 토큰, OAuth secret, 사용자 식별자, 비밀번호를 기록하지 않는다.
- 기존 시각 디자인(프레임, 코너 장식, GALLERY 타이포그래피, 색상, 반응형 높이)은 보존한다.

### 문제별 수정 계획

| 문제 | 원인 | 수정 |
| --- | --- | --- |
| 다음 슬라이드가 정상적으로 노출되지 않거나 빈 구간이 보일 수 있음 | 수동 `translate3d` 루프에서 이동 트랙 자체에 클리핑이 적용되어 viewport와 rail의 책임이 섞여 있음 | Swiper의 `.swiper > .swiper-wrapper > .swiper-slide` 구조를 사용하고, viewport는 `.swiper`가 담당한다. |
| 마지막 슬라이드 이후의 연결이 수동 복제본과 타이머 시간에 의존함 | 복제 슬라이드와 380ms 전환 완료 시간을 앱 코드가 직접 관리함 | 원본 슬라이드만 남기고 `loop: true`의 복제·복귀 처리를 Swiper에 위임한다. |
| 자동재생 요구(800ms)를 유지할 명확한 소유자가 없음 | 수동 `setInterval`이 전환 상태와 독립적으로 실행됨 | Swiper `autoplay.delay: 800`, `disableOnInteraction: false`로 단일 인스턴스에서 관리한다. |
| 외부 의존성 사용 시 CSP 불일치 가능성 | 현재 `script-src`에 Swiper CDN이 없음 | Swiper의 명시적 CDN 출처만 CSP와 stylesheet/script 태그에 추가한다. 다른 외부 출처는 추가하지 않는다. |

### Process Phase 및 Gate

1. **문서 Gate**
   - 이 절이 먼저 `drill`에 커밋되어야 한다.
   - 범위·보안 규칙·검증 기준이 모두 기록되어야 다음 단계로 진행한다.
2. **구조 Gate**
   - `index.html`은 원본 슬라이드만 포함한 Swiper 표준 계층을 사용한다.
   - `app.js`에서 기존 수동 인덱스, interval, 복제 슬라이드 보정 함수가 제거된다.
3. **동작 Gate**
   - 800ms 간격으로 자동 재생되고, 마지막 다음에 첫 슬라이드가 빈 화면·점프 없이 이어진다.
   - 사용자 상호작용 후에도 자동 재생은 유지된다.
   - `prefers-reduced-motion: reduce`에서는 자동 재생하지 않는다.
4. **표시·호환 Gate**
   - 데스크톱과 620px 이하 화면에서 배너 프레임, 장식, 높이, 콘텐츠가 넘치지 않는다.
   - 콘솔 오류·CSP 차단·네트워크 실패가 없다.
5. **품질·배포 Gate**
   - 변경 파일만 커밋하고, 문법·정적·브라우저 검증 및 보안 diff 점검을 통과한다.
   - `main` 병합 후 GitHub Pages 배포 성공을 확인한다.
...
구조/동작/표시/보안 검증 실패
  ...
  → 최소 범위 수정
  → 해당 Gate부터 재검증
  → 모든 Gate 통과 시에만 다음 Phase 및 병합
```

- 외부 의존성 로드가 CSP 또는 네트워크 정책과 충돌하면, 허용 출처를 넓히지 않고 로컬 배포 방식으로 전환할지 별도 승인받는다.
- 기존 UI·인증·Worker 기능에 회귀가 발견되면 해당 변경을 되돌리고 갤러리 범위에서만 재수정한다.

### 검증 절차

- HTML 구조와 JavaScript 문법을 확인한다.
- 로컬 HTTP 환경에서 초기 렌더, 800ms 자동재생, 최소 3회 이상의 루프 경계, 사용자 상호작용 이후 자동재생, reduced-motion을 확인한다.
- 일반·모바일 뷰포트에서 스크린샷으로 프레임 및 슬라이드 잘림을 확인한다.
- CSP가 Swiper 외의 새 출처를 허용하지 않는지 확인한다.
- `drill` diff에 대해 보안 회귀를 점검하고, `main` 병합·배포 뒤 원격 배포 상태를 확인한다.
