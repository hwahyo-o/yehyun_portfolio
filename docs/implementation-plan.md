...
1. 최초 화면 표시 시점부터 로딩 화면을 정확히 2초간 노출한다.
2. Portfolio와 2026 텍스트가 나타난 후 Portfolio 하이라이트가 왼쪽에서 오른쪽으로 채워진다.
3. 하이라이트 이후 Portfolio와 2026의 폰트를 100ms 주기로 셔플한다.
...
- 앱 시작 후 로딩 화면을 먼저 마운트하고 2000ms 뒤 홈으로 전환

## 실행 단계와 Gate

1. 초기 구조: 실행 가능한 앱, 문서, 안전한 기본 스타일
2. 로딩: 텍스트·하이라이트·100ms 폰트 셔플·2000ms 전환
