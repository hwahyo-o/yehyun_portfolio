...
5. 화면과 작업물 기본 상태는 `index.html`의 정적 마크업으로 유지하고, 실제 작업물 자산은 HTML에 연결한다.
...
- 처리: `app.js`의 3초 로딩, 300ms 셔플, 해시 라우팅, 버튼 동작, 32px/400ms About Me 입력 제한
...
2. Phase B — 정적 구조 전환: HTML에 화면 마크업을 배치하고 CSS와 동작 전용 JS로 분리하며 React/Vite 파일과 의존성을 제거한다.
